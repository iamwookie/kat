...
class NebulaLinkSession {
    ...
    constructor(client, int, id, expiry) {
        ...
        setTimeout(() => {
            ...
            if (this.prompt) {
                let expired = new ActionEmbed('fail', 'Session has expired!', this.user);
    ...
    static async initialize(client, int) {
        try {
            ...
            session.pubsub.subscribe(client.database.withPrefix('nebula-link'), (message) => {
            ...
            let fail = new ActionEmbed('fail', 'Something went wrong. Try again later!', int.user);
    ...
    async createPrompt() {
        ...
        await this.user.send({ embeds: [prompt] }).then(int => {
            this.prompt = int;
            let success = new ActionEmbed('success', 'A verification prompt has been sent to your DMs!', this.user);
            this.int.editReply({ embeds: [success] });
        }).catch(() => {
            let fail = new ActionEmbed('fail', 'I could not send you a DM. Are you sure your DMs are open?', this.user);

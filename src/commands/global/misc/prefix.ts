...
export class PrefixCommand extends Command {
    ...
    async execute(int: ChatInputCommandInteraction<"cached"> | Message) {
        if (!int.member?.permissions.has(PermissionFlagsBits.Administrator)) return this.reply(int, { embeds: [new ActionEmbed("fail").setText("You do not have permission to use this command!")] });
        if (!this.client.prisma) return this.reply(int, { embeds: [new ActionEmbed("fail").setText("An error occured while setting the prefix!")] });

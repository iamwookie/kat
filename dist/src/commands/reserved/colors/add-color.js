...
export class AddColorCommand extends Command {
    ...
    async execute(client, int) {
        ...
        if (!roleId)
            return this.reply(int, { embeds: [new ActionEmbed("fail").setDesc("Please provide a role ID!")] });
        this.applyCooldown(author);
        const role = await int.guild?.roles.fetch(roleId);
        if (!role)
            return this.reply(int, { embeds: [new ActionEmbed("fail").setDesc("Invalid role ID!")] });
        const colors = client.colors.get(int.guildId);
        if (colors && Object.values(colors).includes(role.id))
            return this.reply(int, { embeds: [new ActionEmbed("fail").setDesc("This role is already a color!")] });
